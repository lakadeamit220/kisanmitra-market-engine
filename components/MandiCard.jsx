      ...
      <div className={`flex items-center justify-between px-4 py-2.5 ${
        ...
        {isRecommended && (
          <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-brand-100">
            <TrendingUp size={11} /> {t('best_option')}
          </span>
        )}
        {!isRecommended && (
      ...
      <div className="px-4 py-3">
        ...
        {isRecommended && (
          <div className="flex items-center gap-1 text-xs text-slate-400 mb-3">
            <MapPin size={11} />
            <span>{mandi.distanceKm} km {t('away')} &middot; {t('headline')} ₹{mandi.headlinePrice}/{t('qtl')}</span>
          </div>
        )}
        {!isRecommended && (
          <p className="text-xs text-slate-400 mb-3">{t('headline')} ₹{mandi.headlinePrice}/{t('qtl')}</p>
        )}

        {/* Deductions grid */}
        <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 mb-3">
          {[
            { icon: Truck, label: t('transport'), value: mandi.transportCostPerQtl },
            { icon: BadgePercent, label: t('market_fee'), value: mandi.marketFeePerQtl },
            { icon: CircleDollarSign, label: t('commission'), value: mandi.commissionPerQtl },
            { icon: MoveDown, label: t('loading'), value: mandi.loadingChargePerQtl },
        ...
        <div className={`rounded-lg px-3 py-2.5 flex items-center justify-between ${
          ...
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">{t('net_realization')}</p>
            <p className="text-[10px] text-slate-400">{t('after_deductions')}</p>
          </div>
          <div className="text-right">
            <p className={`text-xl font-black ${isRecommended ? 'text-brand-700' : 'text-slate-800'}`}>
              ₹{mandi.netPerQtl.toLocaleString('en-IN')}
            </p>
            <p className="text-[10px] text-slate-400">{t('per_quintal')}</p>
          </div>
        </div>

        {/* Total row */}
        <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-100">
          <span className="text-xs text-slate-400 font-medium">{t('total_earnings')}</span>
